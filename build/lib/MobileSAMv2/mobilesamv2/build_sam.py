...
from perception.MobileSAM.MobileSAMv2.tinyvit.tiny_vit import TinyViT#11000 
from perception.MobileSAM.MobileSAMv2.efficientvit.models.efficientvit.backbone import EfficientViTLargeBackbone
from perception.MobileSAM.MobileSAMv2.efficientvit.models.efficientvit.sam import SamNeck, EfficientViTSamImageEncoder
from perception.MobileSAM.MobileSAMv2.efficientvit.models.nn.norm import set_norm_eps
...
def build_sam_vit_t_encoder(checkpoint=None):
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            state_dict = torch.load(f)
        # Check the available keys in the state_dict
        if 'model' in state_dict:
            mobile_sam.load_state_dict(state_dict['model'], strict=False)
        elif 'state_dict' in state_dict:
            mobile_sam.load_state_dict(state_dict['state_dict'], strict=False)
        else:
